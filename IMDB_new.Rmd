...
##filtering people at least appear 3 times in different movie
```{r}
crew <- df %>% 
  filter(titleType == "movie") %>%  
  filter(numVotes >= 10) %>% 
...
#Clustering for crews(looks bad)
```{r}
